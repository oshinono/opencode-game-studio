...
- Pins the engine and version in `OPENCODE.md`
...
The directories listed in `OPENCODE.md` don't all exist yet. Create them as
...
```
Ask Opencode to create a game concept document using the template at
...
```
Ask Opencode to create a game pillars document using the template at
...
```
Ask Opencode to create a GDD using the template at
...
```
Ask Opencode to create a milestone definition using the template at
...
3. Opencode identifies the right agent(s) to help
...
```
Ask Opencode to create a post-mortem using the template at
...
| Write or review GML code | `gamemaker-gml-specialist` | 3 |
